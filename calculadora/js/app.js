...
function configPadrao() {
  return { nome: NOME_PADRAO, logo: null, banner: null, armas: ARMAS_PADRAO.map((a) => ({ ...a })) };
}
...
document.getElementById("btnZerar").addEventListener("click", () => {
  ...
  document.querySelectorAll('[data-role="preco"]').forEach((select) => (select.selectedIndex = 0));
  calcular();
...
btnSalvar.addEventListener("click", async () => {
  ...
  if (webhook) {
    ...
    try {
      ...
      btnSalvar.textContent = "FINALIZAR COMPRA";
    }
  } else {
...
function montarEmbedDiscord(r) {
  ...
  const campos = [
    ...
    { name: "Comprador(es)", value: r.compradores || "-", inline: true },
    { name: "Vendedor", value: `${r.vendedor || "-"} — <@${r.discordIdVendedor}>`, inline: false },
    ...
    embeds: [
      {
        ...
        color: 13214247, // dourado do site (#c9a227)
        fields: campos,
